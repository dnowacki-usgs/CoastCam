...
def lazyrun(metadata, intrinsics_list, extrinsics_list, local_origin, t, z):
    ...
    if USE_GNSS and not USE_SPLINE:
        ...
        ofile = fildir + 'rect/constantwl/' + product + '/' + t + pd.Timestamp(int(t), unit='s').strftime('.%a.%b.%d_%H_%M_%S.GMT.%Y.golovin') + '.' + camera + '.' + product + '.png'
    else:
...
ts1 = [os.path.basename(x).split('.')[0] for x in glob.glob(fildir + 'products/1*.golovin.c1.'+ product + '.jpg')]
ts2 = [os.path.basename(x).split('.')[0] for x in glob.glob(fildir + 'products/1*.golovin.c2.' + product + '.jpg')]
